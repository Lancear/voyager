...
export function FileTreeEntry({ entry, openFile }: FileTreeEntryProps) {
  const isFolder = Boolean(entry.tree && entry.tree.size > 0);
  const [folderItemsShown, showFolderItems] = createSignal(false);

  return (
    <div class="py-0.5 px-1" data-path={entry.path}>
      <div 
        onClick={isFolder ? () => showFolderItems(o => !o) : () => openFile(entry)}
        class={cls(
      ...
      >
        <svg
          class={cls("size-3.5 shrink-0", folderItemsShown() && "rotate-90")}
      ...
      <Show when={isFolder && folderItemsShown()}>
        <div class="pl-2">
